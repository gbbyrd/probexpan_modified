...
if __name__ == '__main__':
    ...
    parser.add_argument('-num_model', default=5,type=int)  ##
    parser.add_argument('-num_top_model', default=2, type=int)
    ...
    #print(type(args.num_model)) #
    #print(args.num_model)  #
    ...
    if args.pretrained_model is not None:
        ...
        if args.ensemble:
            for i in model_ids:
                print('[Model %d]' % i)
                if args.CL:
                    expan.pretrain(args.save_path+str(i), lr=3e-5, epoch=4, batchsize=256, num_sen_per_entity=256, smoothing=0.075)
                else:
                    expan.pretrain(args.save_path+str(i), lr=2e-5, epoch=3, batchsize=128, num_sen_per_entity=256, smoothing=0.075)
                print('')
            exit(0)
        else:
            if args.CL:
                expan.pretrain(args.save_path, lr=3e-5, epoch=4, batchsize=256, num_sen_per_entity=256, smoothing=0.075)
            else:
                expan.pretrain(args.save_path, lr=2e-5, epoch=3, batchsize=128, num_sen_per_entity=256, smoothing=0.075)
            exit(0)
    if args.ensemble:
